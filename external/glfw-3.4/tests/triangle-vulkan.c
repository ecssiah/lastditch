...
static void demo_init_vk_swapchain(struct demo *demo) {
    ...
    uint32_t graphicsQueueNodeIndex = std::numeric_limits<u32>::max();
    uint32_t presentQueueNodeIndex = std::numeric_limits<u32>::max();
    for (i = 0; i < demo->queue_count; i++) {
        if ((demo->queue_props[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) != 0) {
            if (graphicsQueueNodeIndex == std::numeric_limits<u32>::max()) {
    ...
    if (presentQueueNodeIndex == std::numeric_limits<u32>::max()) {
    ...
    if (graphicsQueueNodeIndex == std::numeric_limits<u32>::max() ||
        presentQueueNodeIndex == std::numeric_limits<u32>::max()) {
