...
#include "edit/sim/actor.h"
#include "edit/sim/physics.h"
...
static void actors_init(Sim *sim)
{
    ...
    Actor editor;
    editor.actor_type = ACTOR_TYPE_EDITOR;
    editor.movement_type = MOVEMENT_TYPE_FLYING;
    
    editor.box_collider.radius[0] = 0.4f;
    ...
    editor.rotation[2] = 90.0f;

    editor.speed = EDITOR_DEFAULT_FLY_SPEED;

    editor.velocity[0] = 0.0f;
    editor.velocity[1] = 0.0f;
    editor.velocity[2] = 0.0f;

    editor.is_grounded = FALSE;

    sim->editor_handle = actor_add_to_pool(&editor, &sim->actor_pool);

    LOG_INFO("Generated Editor, Index: %u", sim->editor_handle.index);
}
...
static void apply_jump_action(Sim *sim, Actor *actor, Action *action)
{
    ...
    {
        actor->velocity[2] = EDITOR_DEFAULT_JUMP_SPEED;
    }
}
...
static void apply_debug_mode_action(Sim *sim, Actor *actor, Action *action)
{
    ...
    case MOVEMENT_TYPE_GROUND:
    {
        ...
        actor->speed = EDITOR_DEFAULT_FLY_SPEED;
    ...
    case MOVEMENT_TYPE_FLYING:
    {
        ...
        actor->speed = EDITOR_DEFAULT_MOVE_SPEED;
...
{
    Actor *actor = &sim->actor_pool.actor_array[sim->editor_handle.index];
