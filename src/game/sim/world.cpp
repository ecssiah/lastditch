...
void 
world_sector_index_to_coordinate(u32 sector_index, ivec2 out_sector_coordinate)
{
...
world_cell_coordinate_to_index(i32 x, i32 y, i32 z)
{
    ...
    assert(cell_index < static_cast<i32>(WORLD_VOLUME_IN_CELLS));

    return cell_index;
}

void 
world_cell_index_to_coordinate(u32 cell_index, ivec3 out_cell_coordinate)
{
...
void 
world_cell_coordinate_to_sector_coordinate(i32 x, i32 y, ivec2 out_sector_coordinate)
{
...
void 
world_cell_coordinate_to_local_coordinate(i32 x, i32 y, i32 z, ivec3 out_local_coordinate)
{
...
void
world_cell_coordinate_to_position(i32 x, i32 y, i32 z, vec3 out_position)
{
    out_position[0] = static_cast<f32>(x);
    out_position[1] = static_cast<f32>(y);
    out_position[2] = static_cast<f32>(z);
}

void 
world_position_to_cell_coordinate(f32 x, f32 y, f32 z, ivec3 out_cell_coordinate)
{
    out_cell_coordinate[0] = static_cast<i32>(floorf(x));
    out_cell_coordinate[1] = static_cast<i32>(floorf(y));
    out_cell_coordinate[2] = static_cast<i32>(floorf(z));
}
...
place_area(World* world, Area* area)
{
    ...
    while (pool_index < area_pool->active_count)
    {
        ...
        if (bounds2i_overlaps(&area_test->bounds, &area->bounds))
        {
            ...
            for (u32 bounds_index = 0; bounds_index < bounds_count; ++bounds_index)
            {
                Area new_area = {
...
{
    constexpr u32 roof_area_size = TOWER_SIZE / 8;
...
static AreaOverlap 
get_area_overlap(const Area* area_left, const Area* area_right)
{
...
static void 
calculate_area_edges(World* world, u32 floor_number)
{
...
static void 
construct_areas(World* world, u32 floor_number)
{
...
static void 
place_content(World* world, u32 floor_number)
{
    const AreaPool* area_pool = &world->area_pool_array[floor_number];
...
{
    constexpr u32 debug_floor_number = TOWER_FLOOR_COUNT;
