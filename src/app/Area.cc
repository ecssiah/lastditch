...
{
    const i32 area_id = area_pool.free_array[--area_pool.free_count];
...
void 
area_remove(AreaPool& area_pool, const i32 area_id)
{
    const i32 pool_id = area_pool.active_lookup[area_id];
    const i32 area_id_last = area_pool.active_array[--area_pool.active_count];
...
{
    const i32 edge_id = edge_pool.free_array[--edge_pool.free_count];
...
void 
area_remove_edge(EdgePool& edge_pool, const i32 edge_id)
{
    const i32 pool_id = edge_pool.active_lookup[edge_id];
    const i32 edge_id_last = edge_pool.active_array[--edge_pool.active_count];
